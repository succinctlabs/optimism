...
func (p *PrefetchingEthClient) start() {
	for {
		select {
		case <-p.runningCtx.Done():
			if p.drain {
				for len(p.windowCh) > 0 {
					window := <-p.windowCh
					p.doFetchWindow(window[0], window[1])
				}
			}

			close(p.closed)
			return
		case window := <-p.windowCh:
			p.doFetchWindow(window[0], window[1])
		}
	}
}
...
func (p *PrefetchingEthClient) fetchWindow(start, end uint64) {
	...
	select {
	case p.windowCh <- windowTuple{start, end}:
		return
	default:
		return
	}
...
func (p *PrefetchingEthClient) doFetchWindow(start, end uint64) {
...
func (p *PrefetchingEthClient) FetchBlockAndReceipts(ctx context.Context, number uint64) {
	...
	blockInfo, _, err := p.inner.InfoAndTxsByNumber(ctx, number) // We need err here, though, to make sure blockInfo is safe to access
	if err != nil {
