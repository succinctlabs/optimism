...
library GameTypes {
    /// @notice The game will use a `IDisputeGame` implementation that utilizes fault proofs.
    GameType internal constant FAULT = GameType.wrap(0);

    /// @notice The game will use a `IDisputeGame` implementation that utilizes validity proofs.
    GameType internal constant VALIDITY = GameType.wrap(1);

    /// @notice The game will use a `IDisputeGame` implementation that utilizes attestation proofs.
    GameType internal constant ATTESTATION = GameType.wrap(2);

    /// @notice A dispute game type that performs output bisection and then uses the cannon vm.
